...
sub tell_nagios {
    ...
    my $perfdata = "|Used=${used}KB;;;; Free=${free}KB;;;; Caches=${caches}KB;;;;";
...
# Show usage
sub usage() {
    print "\n$SCRIPT_NAME v$SCRIPT_VERSION - Nagios Plugin\n";
    print "usage: $SCRIPT_NAME -<f|u> -w <warning> -c <critical>\n\n";
    print "options:\n";
...
sub init {
    ...
    if (!$opt_w or $opt_w == 0 or !$opt_c or $opt_c == 0) {
        print "Usage error: You must define WARN and CRITICAL levels!\n";
        &usage;
    }
    elsif (!$opt_f and !$opt_u) {
        print "Usage error: You must select to monitor either USED or FREE memory!\n";
        &usage;
    }

    # Check if levels are sane
    if ($opt_w <= $opt_c and $opt_f) {
        print "Usage error: WARN level must not be less than CRITICAL when checking FREE memory!\n";
        &usage;
    }
    elsif ($opt_w >= $opt_c and $opt_u) {
        print "Usage error: WARN level must not be greater than CRITICAL when checking USED memory!\n";
        &usage;
    }
}
